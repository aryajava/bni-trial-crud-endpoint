...
public interface IOrderService
{
    ...
    Task<(bool Success, string? Error)> PackAsync(long id, string updatedBy);
    Task<(bool Success, string? Error)> ShipAsync(long id, string updatedBy);
    Task<(bool Success, string? Error)> CancelAsync(long id, string reason, string updatedBy, bool fromPenjual = false);
